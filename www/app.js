...
app.initialize = function()
{
    ...
    var notEnabled = function() {
    	...
    	bluetoothSerial.enable(
    		    ...
    		    function() {
    		    	navigator.notification.alert("You must turn on bluetooth to start scanning devices!");
    		    }
    		);
    }
...
//		app.updateConversation(message, false);
	...
	{
		// Disconnect and show an error message to the user.
		app.disconnect('sendMessage Disconnected');
...
app.startScan = function()
{
	...
	{
		// Show an error message to the user
		app.disconnect('Start Scan Failed to scan for devices.');
...
app.receivedMessage = function(data)
{
	if (app.connected)
	{
		...
		var message = String.fromCharCode.apply(null, new Uint8Array(data)).trim();
...
//		app.updateConversation(message, true);
		...
		for (var key in dataBase) {
			...
			if (message == key) {
				console.log("true");
				$('#conversation').text(dataBase[key]);
				break;
			}
			else
